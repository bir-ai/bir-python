...
The easy path is the `bir.logging` filter. Configure logging, attach
`BirTraceIdFilter` once with `install_trace_id_filter()`, and every log record gains
`bir_trace_id` / `bir_span_id` attributes that any formatter can render — no
per-call plumbing:
...
def answer(question: str) -> str:
    logging.getLogger("myapp").info("handling question")  # ids stamped automatically
    return "ok"
```

Configure logging first. A filter on a *logger* only sees records that logger
creates, so the stamp has to reach the *handlers* your loggers propagate to, and
those handlers have to exist when you call this — a no-argument call with no
handlers on the root logger warns rather than attaching to nothing. Add a handler
later and you pass it yourself: `install_trace_id_filter(handler)`.
...
They are also re-created in a forked child through `os.register_at_fork`, so a
pre-forking server or a `multiprocessing` pool started from a process that was
recording gets workers that record instead of workers that block on a lock the
parent's threads still hold. What a child inherits — the trace it was forked
inside of, and the configured store path — is described under
[recording in a process that forks](docs/site/core-api.md#recording-in-a-process-that-forks).
...
conventions as of **opentelemetry-semantic-conventions 0.65b0** —
`gen_ai.request.model`, `gen_ai.usage.input_tokens` /
`gen_ai.usage.output_tokens`, and both `gen_ai.provider.name` and the spelling
it superseded, `gen_ai.system`, on a generation whose provider was recorded
(e.g. by the LiteLLM or Pydantic AI integrations; it is never guessed from the
model string) — with `bir.*` attributes for the rest (event type, score value,
token totals, cost, and the originating Bir ids). Pass `headers=` for backend auth, or inject your own configured
...
`deployment.environment.name`, the superseded `deployment.environment`, and
`bir.source`. Each is added only when a single value
applies to the whole export. Pass `environment="prod"` to set
the deployment environment explicitly (it overrides whatever the traces recorded). If
...
repeatable for backend auth, and `--service-name`, `--environment` (sets the
