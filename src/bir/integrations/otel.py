...
``gen_ai.usage.output_tokens``) with ``bir.*`` attributes for everything else.
...
def export_traces_to_otlp(
    ...
    *,
    ...
    resource = api.Resource.create({api.SERVICE_NAME: service_name})
    ...
    try:
        exported = 0
        for trace in loaded:
            exported += _export_one_trace(trace, tracer=tracer, api=api)
...
def _export_one_trace(trace: LoadedTrace, *, tracer: Any, api: _OtelApi) -> int:
    ...
    spans_by_event_id[root_event.id] = _emit_span(root_event, parent_span=None, tracer=tracer, api=api)
    ...
    while queue:
        ...
        for child in children_by_parent.get(parent_event.id, ()):
            ...
            spans_by_event_id[child.id] = _emit_span(
                child, parent_span=spans_by_event_id[parent_event.id], tracer=tracer, api=api
            )
            queue.append(child)
    ...
    for event in trace.events:
        if event.id not in spans_by_event_id:
            spans_by_event_id[event.id] = _emit_span(event, parent_span=root_span, tracer=tracer, api=api)

    return len(spans_by_event_id)


def _emit_span(event: TraceEvent, *, parent_span: Any | None, tracer: Any, api: _OtelApi) -> Any:
...
def _event_attributes(event: TraceEvent) -> dict[str, Any]:
    ...
    and ``gen_ai.usage.input_tokens`` / ``gen_ai.usage.output_tokens``); the
